...
async function discoverTotalPages() {
    ...
    if (!PROXY_HOST || !PROXY_PORT) {
        console.error("Lỗi: Trinh sát viên không được trang bị proxy.");
        return 1;
    }
    
    const browserArgs = ['--no-sandbox', '--disable-setuid-sandbox', `--proxy-server=${PROXY_HOST}:${PROXY_PORT}`];

    try {
        console.error(`[Thám tử] Khởi tạo trình duyệt với danh tính ${PROXY_HOST}...`);
