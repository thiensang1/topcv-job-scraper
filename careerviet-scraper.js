...
async function scrapeHTML(page, pageNum, retry = 0) {
    ...
    try {
        ...
        // Debug cho trang đầu, retry, và bội số 50
        if (pageNum <= 2 || retry > 0 || pageNum % 50 === 0) {
...
(async () => {
    ...
    try {
        ...
        while (pageNum <= MAX_PAGES) {
            ...
            // Delay ngẫu nhiên 10-20s
            const betweenPagesDelay = Math.floor(Math.random() * (20000 - 10000 + 1)) + 10000;
            console.error(` -> Nghỉ ${betweenPagesDelay / 1000}s trước trang tiếp theo...`);
