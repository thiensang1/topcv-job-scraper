...
async function scrapeTopCVByKeyword(keyword, pageNum) {
    ...
    const launchOptions = {
        headless: true,
        // THAY ĐỔI 1: Thêm args để chạy trên GitHub Actions
    ...
    try {
        ...
        // Nếu có lỗi (như timeout), trả về mảng rỗng để script có thể tiếp tục hoặc dừng lại một cách an toàn
        return [];
    } finally {
        await browser.close();
    }
...
(async () => {
    ...
    if (allJobs.length > 0) {
        // THAY ĐỔI 2: Tạo tên file an toàn và lưu vào thư mục data
