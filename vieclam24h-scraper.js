...
async function getAllJobLinks(browser) {
    ...
    while (hasNextPage) {
        ...
        try {
            ...
            const linksOnPage = await page.$$eval('div.box-job-info h3.title-job a', anchors => anchors.map(a => a.href));
            ...
            hasNextPage = false; // Dừng lại nếu có lỗi
        } finally {
...
async function scrapeJobDetails(url, browser) {
    ...
    try {
        ...
        const location = $('span[data-id="Location"]').text().trim() || 'Không xác định';
        const postedDateText = $('span[data-id="PostedDate"]').text().trim() || null;

        return {
            'Tên công việc': title, 'Tên công ty': company,
            'Nơi làm việc': location, 'Mức lương': salary,
            'Ngày đăng tin': postedDateText, 'Link': url
        };
    } catch (error) {
